...
# Novas colunas de dimensão da produção (adicionadas para o cálculo correto)
...
def load_estoque_inicial_bruto():
    ...
    try:
        ...
        query_inicial = f"""
            SELECT
                "{COL_ESTOQUE_PRODUTO}" as Produto,
                "{COL_ESTOQUE_PERFIL}" as Perfil,
...
def load_producao_bruta():
    ...
    try:
        ...
        # O CÁLCULO DE PEÇAS FOI ALTERADO AQUI
        query_producao = f"""
            SELECT
                "{COL_PRODUCAO_PRODUTO}" as Produto,
                "{COL_PRODUCAO_PERFIL}" as Perfil,
                
                -- LÓGICA ANTIGA (ERRADA PARA PEÇAS SOLTAS)
                -- SUM("{COL_PRODUCAO_PECAS}" * "{COL_PRODUCAO_PACOTES}") as Total_Pecas,
                
                -- LÓGICA NOVA (CORRETA)
                -- Calcula M3/Peça e divide o M3 total por ele.
                -- Isso garante que peças soltas (como a Linha 21 de 80 peças) sejam contadas.
                SUM(
                    CASE 
                        -- Se as dimensões e M3 existirem, calcula as peças
                        ...
                        -- Se M3 for 0, mas as peças antigas forem > 0, usa a lógica antiga (backup)
        ...
        df = df.rename(columns={COL_PRODUCAO_PERFIL: 'Perfil'})
        
        # Debug: Mostrar o que foi lido (opcional)
        print("\n[DEBUG] Dados da Produção Corrigida:")
        print(df[df['Produto'] == 'PAINEL RIPADO'])
        
        return df
    except Exception as e:
        print(f"[ERRO] Falha ao ler Producao01 (corrigido): {e}")
        return pd.DataFrame(columns=['Produto', 'Perfil', 'Total_Pecas', 'Total_M3'])
    finally:
...
def load_saidas_brutas_agrupadas():
    ...
    try:
        ...
        query = f"""
            SELECT
                "{COL_SAIDA_PRODUTO}" as Produto,
                "{COL_SAIDA_PERFIL}" as Perfil,
...
def load_pedidos_solicitados_por_po():
    ...
    try:
        ...
        query = f"""
            SELECT
                "{COL_CARTEIRA_PO}",
                "{COL_CARTEIRA_CLIENTE}",
                "{COL_CARTEIRA_PERFIL}",
                "{COL_CARTEIRA_PRODUTO}",
        ...
        return pd.DataFrame(columns=['PO', 'Cliente', 'Perfil', 'Produto', 'Solicitado_Pecas', 'Solicitado_M3', 'Vol_Medio_Peca'])
    finally:
...
def load_pedidos_enviados_por_po():
    ...
    try:
        ...
        return pd.DataFrame(columns=['PO', 'Produto', 'Enviado_Pecas'])
    finally:
        if conn_expedicao: conn_expedicao.close()
...
def exportar_dados():
    ...
    df_producao_bruta = load_producao_bruta() # Agora usa a função corrigida
    ...
    # --- 2. Calcular Itens Pendentes ---
    print("Lendo Carteira de Pedidos...")
    df_solicitado_po = load_pedidos_solicitados_por_po()
    print("Lendo Saídas por PO...")
    ...
    print(f"Itm Pendentes calculados ({len(itens_pendentes_json)} linhas).")


    # --- 3. Gravar o arquivo .js ---
    print("\n--- Gravando arquivo ---")
    try:
    ...
    try:
        with open(CAMINHO_SAIDA_JS, 'w', encoding='utf-8') as f:
            ...
            json.dump(estoque_liquido_json, f, ensure_ascii=False, indent=4, cls=NpEncoder)
            f.write(";\n\n")

            f.write("const MOCK_DADOS_PENDENTES_DETALHADOS = ")
            json.dump(itens_pendentes_json, f, ensure_ascii=False, indent=4, cls=NpEncoder)
            f.write(";\n")
...
if __name__ == "__main__":
    print("--- Exportador Automático de Dados (Motor v1.2.0 - Corrigido) ---")
